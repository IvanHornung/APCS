...
public class Ball2
{
    ...
    private Color color;
    private boolean left, right, up, down, isG2;
    ...
    public Ball2(AIPlayer p1, Player p2, Game g, boolean player1Turn)
    {
        size = 15;
        x = (int)((Game.WIDTH)/2);  //( Math.random()*(Game.WIDTH-size) );
        y = (int)((Game.HEIGHT)/2); //( Math.random()*(Game.HEIGHT-size) );
        color = Color.WHITE;
        if(Game.gamemode == "Multiball") color = new Color(255, 204, 153 );
    ...
    public Ball2(Player p1, Player p2, TwoGame g, boolean player1Turn)
    {
        size = 15;
        ...
        color = Color.WHITE;
        if(Game.gamemode == "Multiball") color = new Color(255, 204, 153 );
        if(TwoGame.gamemode != "Accelerating") speed = 3; //(int)( Math.random()*5)+1;
        else {
            speed = -1;
        }
        ...
        ///////////////////////////////Two-Player//////////////////////////
            ...
            if( vx > 0 ) {
                if( (x + size ) >= player.getXPos() &&  (x + size) <= (player.getXPos() + Player.PLAYER_W) && y >= player.getYPos() && y <= (player.getYPos() + Player.PLAYER_H)) {
                    ...
                    new Sound("PlayerHit");
                    hitCount++;
                    if(TwoGame.gamemode == "50CC") player.changeColor();
                ...
                if(x >= playerL.getXPos() && x <= (playerL.getXPos() + Player.PLAYER_W) && y >= playerL.getYPos() && y <= (playerL.getYPos() + Player.PLAYER_H)) {
                    vx *= -1;
                    new Sound("PlayerHit");
                    hitCount++;
                    if(TwoGame.gamemode == "50CC") playerL.changeColor();
